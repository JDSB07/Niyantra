  ...
  return (
    <div className="border border-slate-200 bg-white p-4">
      ...
      <div className="mt-2 flex items-baseline gap-2">
        ...
        <span className="text-xl font-bold text-navy">{after}{suffix}</span>
      </div>
...
export default function WhatIfSimulator() {
  ...
  return (
    <div>
      <div className="mb-6 border-b border-slate-200 pb-4 bg-slate-50 p-5">
        <h2 className="text-xl font-bold text-navy">What-If Simulator</h2>
        <p className="mt-1 text-sm text-slate-500">
          Stage a hypothetical defect or freight surge, then preview how System 2 would replan the week
        </p>
      </div>

      <div className="mb-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="border border-slate-200 bg-white p-5">
          <h3 className="mb-4 text-sm font-bold uppercase tracking-wider text-navy border-b border-slate-200 pb-2">Add a new defect</h3>
          ...
          {stagedDefects.length > 0 && (
            <div className="mt-3 space-y-1.5">
              {stagedDefects.map((d) => (
                <div key={d.task_id} className="flex items-center justify-between rounded-lg bg-cream px-2.5 py-1.5 text-xs">
                  <span className="capitalize text-navy">{d.department} &middot; {d.defect_type.replaceAll('_', ' ')} &middot; {d.section}</span>
        ...
        <div className="border border-slate-200 bg-white p-5">
          <h3 className="mb-4 text-sm font-bold uppercase tracking-wider text-navy border-b border-slate-200 pb-2">Add a freight-forecast surge</h3>
          ...
          {surges.length > 0 && (
            <div className="mt-3 space-y-1.5">
              {surges.map((s) => (
                <div key={s.section} className="flex items-center justify-between rounded-lg bg-cream px-2.5 py-1.5 text-xs">
                  <span className="text-navy">{s.section} &middot; -{s.pct}% window</span>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <button
          ...
          className="focus-ring flex items-center gap-2 rounded-lg bg-gold px-4 py-2 text-sm font-semibold text-navy hover:bg-gold-dark disabled:opacity-40"
        >
          <Play size={14} /> {running ? 'Running simulation…' : 'Run simulation'}
        </button>
        {(stagedDefects.length > 0 || surges.length > 0) && (
          <button onClick={discard} className="focus-ring flex items-center gap-1.5 text-xs text-slate-400 hover:text-navy">
        ...
        {simResult && (
          <div className="border border-slate-200 bg-white p-6 shadow-sm">
            <h3 className="mb-4 text-sm font-bold uppercase tracking-wider text-navy border-b border-slate-200 pb-2">Before / After this week's schedule</h3>
            ...
            {sectionsImpacted.length > 0 && (
              <p className="mt-4 text-sm text-slate-500">
                Sections impacted: <span className="font-medium text-navy">{sectionsImpacted.join(', ')}</span>
              </p>
            )}
              ...
              <div>
                ...
                <div className="space-y-1">
                  {shiftedTasks.newlyScheduled.map((tid) => (
                    <div key={tid} className="rounded-lg bg-cream px-2.5 py-1.5 font-mono text-xs text-navy">{tid}</div>
                  ))}
                </div>
              </div>
              <div>
            ...
            <div className="mt-5 flex gap-2">
              ...
              <button
                onClick={discard}
                className="focus-ring rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-navy hover:bg-cream"
              >
                Discard
              </button>
            </div>
            <p className="mt-3 text-[11px] text-slate-400">
              Note: running a simulation does call the live optimizer, so it's written to the backend as pending
              blocks — it just won't appear in your workspace views unless you commit it here.
            </p>
          </div>
        )}
    </div>
