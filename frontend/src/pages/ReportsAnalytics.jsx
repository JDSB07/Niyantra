...
export default function ReportsAnalytics() {
  ...
  // Downtime trend across the 4-week horizon (avg scheduled-block duration per week)
  ...
  // SLA compliance proxy: share of each department's backlog that made it
  // into this week's schedule (no explicit SLA-clock field exists yet).
  ...
  // Block utilization: actual scheduled minutes vs. total available corridor
  // window (window_minutes − buffer_minutes) this week.
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-none border-2 border-slate-300 dark:border-slate-700 shadow-sm min-h-full transition-colors">
      <div className="border-b-4 border-ir-maroon p-4 flex flex-wrap items-start justify-between gap-3 bg-ir-cream dark:bg-slate-800">
        <div>
          <h2 className="text-xl font-black text-ir-maroon dark:text-ir-gold uppercase tracking-wide">{t('rep.title')}</h2>
          <p className="mt-1 text-xs font-semibold text-slate-600 dark:text-slate-400">{t('rep.subtitle')}</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={exportCsv}
            className="focus-ring flex items-center gap-2 rounded-none border-2 border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-1.5 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"
          >
          ...
          <button
            ...
            className="focus-ring flex items-center gap-2 rounded-none bg-ir-maroon px-3 py-1.5 text-xs font-bold text-white transition hover:bg-ir-darkmaroon disabled:opacity-50"
          >
