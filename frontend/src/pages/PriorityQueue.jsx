...
function SortHeader({ label, sortKey, sortBy, sortDir, onSort, className = '' }) {
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1 text-left text-xs font-semibold uppercase tracking-wide text-slate-400 hover:text-navy ${className}`}
    >
      {label}
    ...
    <>
      <tr className="border-b border-slate-100 hover:bg-slate-50">
        <td className="py-2 pl-4 pr-2">
          <button onClick={() => setOpen(!open)} className="text-slate-400 hover:text-navy">
            {open ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
          </button>
        </td>
        <td className="py-2 px-2 font-mono text-xs text-navy">{task.task_id}</td>
        ...
        <td className="py-2 px-2 text-xs font-semibold text-navy">{task.risk_score}</td>
            ...
            <div className="flex gap-6">
              ...
              <div className="flex w-64 flex-col gap-2 border-l border-slate-200 pl-6">
                <button
                  ...
                  className="focus-ring flex items-center gap-1.5 rounded bg-navy px-3 py-1.5 text-xs font-medium text-white hover:bg-navy-lighter disabled:cursor-not-allowed disabled:opacity-40"
                >
                  <CheckCircle2 size={13} /> Approve slot
                </button>
                <button
                  onClick={onSendToScheduler}
                  className="focus-ring flex items-center gap-1.5 rounded border border-slate-300 bg-white px-3 py-1.5 text-xs font-medium text-navy hover:bg-slate-100"
                >
                ...
                <button
                  ...
                  className={`focus-ring flex items-center gap-1.5 rounded border px-3 py-1.5 text-xs font-medium ${
                    flagged ? 'border-amber-200 bg-amber-50 text-amber-900' : 'border-slate-300 bg-white text-navy hover:bg-slate-100'
                  }`}
                >
...
function Metric({ label, value }) {
  return (
    <div>
      ...
      <p className="mt-0.5 text-xs font-medium text-navy capitalize">{value}</p>
    </div>
  )
}
...
export default function PriorityQueue() {
  const {
    rankedTasks, blocks, priorityLoading, runPrioritize, runOptimizeWeekly, error,
  ...
  return (
    <div className="bg-white rounded border border-slate-200 shadow-sm min-h-full">
      <div className="border-b border-slate-200 p-5 flex flex-wrap items-start justify-between gap-3 bg-slate-50">
        <div>
          <h2 className="text-xl font-bold text-navy">Priority Queue</h2>
          <p className="mt-1 text-xs text-slate-500">
            {rankedTasks.length} tasks from TMS/SMMS/TDMS &bull; ranked by System 1 risk model
          </p>
        </div>
        <button
          onClick={() => runPrioritize()}
          disabled={priorityLoading}
          className="focus-ring flex items-center gap-2 rounded bg-navy px-4 py-2 text-xs font-medium text-white transition hover:bg-navy-lighter disabled:opacity-50"
        >
        ...
        {options.map((o) => (
          <button
            ...
            className={`px-3 py-1 text-[11px] font-semibold uppercase tracking-wider transition border ${
              value === o ? 'border-navy bg-navy text-white' : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50'
