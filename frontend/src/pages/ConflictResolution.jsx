  ...
  return (
    <div className="bg-white rounded border border-slate-200 shadow-sm min-h-full">
      <div className="border-b border-slate-200 p-5 bg-slate-50">
        <h2 className="text-xl font-bold text-navy">Conflict Resolution</h2>
      ...
      <div className="p-5">
        <section className="mb-10">
          <h3 className="mb-4 flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-navy border-b border-slate-200 pb-2">
                ...
                <div key={b.block_id} className="border border-slate-200 bg-white">
                  <div className="flex items-center justify-between bg-slate-100 px-4 py-2 border-b border-slate-200">
                    ...
                    <span className="bg-amber-100 border border-amber-200 px-2 py-0.5 text-xs font-semibold text-amber-900">
                      {b.section} &middot; {formatTimeOfDay(b.start_minute)}–{formatTimeOfDay(b.end_minute)}
                    </span>
                  </div>
                    ...
                    {b.task_ids.map((tid) => {
                      ...
                      return (
                        <div key={tid} className="flex items-center justify-between border-b border-slate-100 pb-2 last:border-0 last:pb-0">
                          <div>
                            <p className="font-mono text-xs text-navy font-semibold">{tid}</p>
                            <p className="text-[11px] capitalize text-slate-500">
                              {t.department} &middot; {t.defect_type.replaceAll('_', ' ')}
                            </p>
                          </div>
                  ...
                  <div className="bg-slate-50 px-4 py-3 border-t border-slate-200 flex gap-2">
                    <button
                      ...
                      className="focus-ring flex items-center gap-1.5 bg-navy px-3 py-1.5 text-xs font-semibold text-white hover:bg-navy-lighter disabled:opacity-40"
                    >
                    ...
                    <button
                      ...
                      className={`focus-ring flex items-center gap-1.5 border px-3 py-1.5 text-xs font-semibold ${
                        escalated.has(b.block_id) ? 'border-red-300 bg-red-50 text-red-700' : 'border-slate-300 bg-white text-navy hover:bg-slate-100'
                      }`}
                    >
            ...
            <div className="space-y-6">
              {capacityConflicts.map((group) => (
                <div key={group.section} className="border border-slate-200 bg-white">
                  <div className="bg-slate-100 px-4 py-2 border-b border-slate-200 flex items-center gap-2 text-sm font-bold text-navy">
                        ...
                        {group.losers.map((t) => (
                          <div key={t.task_id} className="flex flex-col gap-1 border border-red-100 bg-white p-2">
                            <div className="flex justify-between items-center">
                              <span className="font-mono text-xs font-bold text-navy">{t.task_id}</span>
                              <span className="text-[10px] bg-red-100 text-red-800 px-1.5 py-0.5 rounded font-bold">Score: {t.risk_score}</span>
                            </div>
                            <span className="text-[11px] text-slate-600 capitalize">{t.department} &middot; {t.defect_type.replaceAll('_', ' ')}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="bg-slate-50 px-4 py-3 border-t border-slate-200 flex flex-wrap items-center gap-3">
                    <button
                      ...
                      className="focus-ring flex items-center gap-1.5 border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-navy hover:bg-slate-100"
                    >
                    ...
                    <button
                      ...
                      className="focus-ring ml-auto text-[11px] font-semibold text-slate-500 uppercase tracking-wider hover:text-navy hover:underline"
                    >
                      View in Queue &rarr;
                    </button>
                  </div>
                  {group.losers.some((t) => accepted.has(t.task_id) || flaggedTaskIds.has(t.task_id)) && (
                    <div className="px-4 py-2 bg-amber-50 text-[11px] font-medium text-amber-800 border-t border-amber-100">
                      {group.losers.filter((t) => accepted.has(t.task_id)).length > 0 && 'Accepted — will roll forward automatically. '}
                      {group.losers.filter((t) => flaggedTaskIds.has(t.task_id)).length > 0 && 'Flagged for manual review.'}
                    </div>
                  )}
                </div>
              ))}
