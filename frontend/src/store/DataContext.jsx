...
export function DataProvider({ children, userContext }) {
  ...
  const value = useMemo(
    () => ({
      ...
      runOptimizeWeekly: (ws = weekStart) => runOptimizeWeekly(rankedTasks.length ? rankedTasks : tasks, corridors, ws),
      runSimulateMonthly: (monthLabel) => runSimulateMonthly(tasks, corridors, monthLabel),
      decideBlock,
      flaggedTaskIds,
      toggleFlag,
      commitSimulation,
    ...
    [
      filteredTasks, corridors, filteredRankedTasks, filteredBlocks, unscheduledTaskIds, monthlyPlan, weekStart,
      bootLoading, priorityLoading, weeklyLoading, monthlyLoading, error,
      runPrioritize, runOptimizeWeekly, runSimulateMonthly, decideBlock,
      flaggedTaskIds, toggleFlag, commitSimulation,
    ]
  )
