...
import { DEPT_COLOR, blockPrimaryColor, formatDayLabel, formatTimeOfDay } from './deptColors.js'
...
// Illustrative line traffic for the selected day — the backend's TMS/COA feed
// doesn't expose real train timings yet, so this renders a stable, plausible
// set of passenger/freight services around the maintenance windows.
...
export default function DayTimeline({ dayDate, onDayChange, blocks, section, onDecide }) {
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <button
            ...
            className="focus-ring flex h-7 w-7 items-center justify-center rounded-full text-slate-400 hover:bg-cream hover:text-navy"
          >
            <ChevronLeft size={16} />
          </button>
          <h3 className="font-serif text-lg font-semibold text-navy">{formatDayLabel(dayDate)}</h3>
          <button
            onClick={() => onDayChange(1)}
            className="focus-ring flex h-7 w-7 items-center justify-center rounded-full text-slate-400 hover:bg-cream hover:text-navy"
          >
            ...
            {traffic.map((tr, i) => (
              <motion.div
                ...
                className={`absolute top-1 flex h-10 items-center justify-center rounded-md border px-1 text-[11px] font-semibold ${
                  tr.isFreight
                    ? 'border-[#AB5A3E]/40 bg-[#FCEEDA] text-[#AB5A3E]'
                  ...
                  <AnimatePresence>
                    {openBlockId === b.block_id && (
                      <motion.div
                        ...
                        className="absolute z-20 mt-1 w-56 rounded-xl border border-slate-100 bg-white p-3 text-xs text-slate-600 shadow-card"
                      >
                        <p className="font-mono text-navy">{b.block_id}</p>
                        <p className="mt-1">{b.section} &middot; {formatTimeOfDay(b.start_minute)}–{formatTimeOfDay(b.end_minute)}</p>
